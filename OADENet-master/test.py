...
def parse_args():
    ...
    parser.add_argument('--model_path', type=str, default='./log/OACC_faster-Net_dis81440.pth')
...
def test(cfg):
    ...
    for scenes in scene_list:
        ...
        if cfg.crop == False:
            ...
            with torch.no_grad():
                disp, _ = net(data.unsqueeze(0).to(cfg.device))
            ...
            with torch.no_grad():
                ...
                for idx_inference in range(num_inference):
                    ...
                    out_disp.append(net(input_data.to(cfg.device))[0])
        ...
        plt.imsave(cfg.save_path + '/%s.png' % (scenes),disp)
